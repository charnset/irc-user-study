...
class MyOwnBot(pydle.Client):
    def __init__(
        ...
        **kwargs
        ...
        super().__init__(
            nickname, fallback_nicknames, username, realname, eventloop, **kwargs
        )
        # Load Puppeteer
        self._puppeteer = load_puppeteer(agenda_dir, agenda_names)
        self._conversation = []
        self._actions = []
    ...
    async def on_message(self, target, source, message):
        ...
        if source != self.nickname:
            ...
            actions, extractions = self._puppeteer.react(
                [MessageObservation(message)], self._extractions
            )
            ...
            if cur_states:
                for agenda_name, at_state in cur_states.items():
                    await self.message(
                        ...
                        + "       DEBUG:   "
                        + "ACTIVE = {}".format(
                            get_agenda_state_log(agenda_name, at_state)
                        ),
                    )
...
def main_event(args):
    ...
    for i in range(n):
        client = MyOwnBot("MyBot" + str(i), agenda_dir, agenda_names)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--agenda_dir", type=str, help="path to agenda & data directory"
    )
    parser.add_argument(
        "--agenda_names", nargs="+", type=str, help="list of agenda names"
    )
