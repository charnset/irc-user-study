...
class MyOwnBot(pydle.Client):
    ...
    async def on_message(self, target, source, message):
        ...
        if source != self.nickname:
            prev_agenda = self._puppeteer.get_current_agenda()
            prev_agenda_name = prev_agenda.name if prev_agenda else None
            prev_state = self._puppeteer.get_current_state(prev_agenda)
            ...
            cur_agenda = self._puppeteer.get_current_agenda()
            cur_agenda_name = cur_agenda.name if cur_agenda else None
            cur_state = self._puppeteer.get_current_state(cur_agenda)
            ...
            #Debugging message
            await self.message(target, "\u0003" + "15" + \
                "DEBUG:   " + \
                "AGENDA={}, STATE={}".format(prev_agenda_name, prev_state) + \
                "   >>>>>>>>>   " + \
                "AGENDA={}, STATE={}".format(cur_agenda_name, cur_state)) #light gray


'''
client = MyOwnBot("MyBot", realname="My Bot")
client.run("128.9.36.104", 6697)
'''
